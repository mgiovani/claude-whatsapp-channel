...
WhatsApp JIDs:
- DMs: `<country+phone>@s.whatsapp.net` — e.g. `5511999999999@s.whatsapp.net`
- Groups: `<id>@g.us` — e.g. `120363xxxxxxxxx@g.us`
...
8. Confirm: who was approved (senderId/JID).
...
2. Set `groups[<groupJid>] = { requireMention: !hasFlag("--no-mention"), allowFrom: parsedList }`.
3. Write.
...
- The `approved/` dir is polled by the server every 5s. After writing the approval
